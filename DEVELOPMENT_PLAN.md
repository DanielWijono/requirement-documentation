...
- **Current phase:** Phase 6m (production deploy).
...
2. Phase 6m: `compose.prod.yaml` with Caddy (TLS, the built web app, `/api` and `/collab` proxied, CSP), a one-shot migrate job, nightly `pg_dump` with a restore runbook, SMTP configuration and environment docs; a local smoke test of the production compose (no real VPS yet).
