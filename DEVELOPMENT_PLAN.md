...
- **Current phase:** Phase 6l (realtime co-editing).
...
2. Phase 6l: Hocuspocus in compose, authorized through the session and `evaluatePageAccess`; a Yjs document per draft; Tiptap Collaboration/Caret; presence avatars; the structural-event banner (§9.3); comment anchors as Yjs relative positions. Then 6m (production compose with Caddy, backups, env docs).
