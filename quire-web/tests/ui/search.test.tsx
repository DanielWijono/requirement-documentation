...
/** Results arrive from the (fake) server; wait until they match. */
const expectTitles = (check: (titles: (string | null)[]) => void) => waitFor(() => check(resultTitles()))

describe('search results (design.md §8.5)', () => {
  it('reads the query from the URL and ranks title matches first', async () => {
    renderApp('/search?q=onboarding')
    expect(screen.getByLabelText('Search this site')).toHaveValue('onboarding')
    await expectTitles((t) => expect(t.slice(0, 2).sort()).toEqual(['New hire onboarding', 'Onboarding']))
  })

  it('snippets come from the published body and highlight the match', async () => {
  ...
  it('filters by label, type and last modified', async () => {
    ...
    await user.click(await within(labels).findByRole('button', { name: 'payments' }))
    await expectTitles((t) => expect(t).toEqual(['ADR-012: Queueing strategy for payment events']))
    ...
    await expectTitles((t) => {
      expect(t.length).toBeGreaterThan(0)
      expect(t).not.toContain('Runbooks')
    })
  ...
  it('zero results names the most restrictive filter and can remove it', async () => {
    ...
    expect(await screen.findByText('Product', { selector: 'strong' })).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Remove it' }))
    await expectTitles((t) => expect(t).toContain('ADR-012: Queueing strategy for payment events'))
  })

  it('sorts by last modified using real ages', async () => {
    const { user } = renderApp('/search')
    await user.click(screen.getByRole('button', { name: 'Last modified' }))
    await expectTitles((titles) => {
